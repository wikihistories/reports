...
wikidata_australians <- readr::read_csv(file.path(data_dir, "australian_wikidata_records.csv"))
```
...
As a first step, we retrieved the Wikidata items for all the pages in `Category:Australian people`, and distinguish them based on whether they have the property P31(Q5), 'instance of: human'.
...
if (USE_CACHE == FALSE) {
  australians_wikidata_items <- purrr::map(
    split(australians$nodes$pageid, seq_along(australians$nodes$pageid) %/% 50),
    get_wikibase_item,
    .progress = T
  ) %>%
    dplyr::bind_rows() %>% 
    readr::write_csv(file.path(data_dir, "australians-wikidata-items.csv"))
} else {
  australians_wikidata_items <- readr::read_csv(file.path(data_dir, "australians-wikidata-items.csv"))
}

```


Although Wikidata indicates humanity clearly, it does not so clearly indicate the Australianness of a person. There are many different Wikidata properties that indicate a person could be Australian. Australia is item [Q408](https://www.wikidata.org/wiki/Q408). The three main ways a person can be linked to Australia in the database is by their citizenship ([P27](https://www.wikidata.org/wiki/P27)), place of birth ([P19](https://www.wikidata.org/wiki/P19)) or place of death ([P20](https://www.wikidata.org/wiki/P20)). 
