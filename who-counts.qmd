...
1. We queried Wikidata for items representing Australian people, and then checked to see if those people had a biographical article in Wikipedia
...
We queried Wikidata by `r FILTER_PROPERTIES %>% nrow() %>% prettyNum(",")` properties that denote Australianness. Using these properties, we were able to identify `r full_dataset %>% filter(dataset != "category_aus") %>% nrow() %>% prettyNum(",")` Wikidata items representing Australian people. Of these Wikidata items, `r full_dataset %>% filter(dataset != "category_aus", on_english_wikipedia) %>% nrow() %>% prettyNum(",")` items had a corresponding Wikipedia article. 

We then used Wikidata to determine which of the `r full_dataset %>% filter(dataset != "wikidata") %>% nrow() %>% prettyNum(",")` articles in `Category:Australian people` were biographies (as opposed to articles about events, things, or groups of people). To do this, we fetched the Wikidata item for each Wikipedia article in the dataset, then checked to see if it had property P31(Q5), 'instance of: human'. When we did this, `r full_dataset %>% filter(dataset != "wikidata", is_human) %>% nrow() %>% prettyNum(",")` articles were left.
